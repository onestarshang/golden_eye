...
save_dir = '/root/macd_backtest_data'
...
def main(ifcode, period_short, period_long):
    ...
    for day in date_list:
        print '*'*20
        print day
        df = FittingDataCalculator.macd_df(day, ifcode, period_short, period_long)
        save_data(df.values.tolist(), day, ifcode, period_short, period_long)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="save macd dataframe to files")
